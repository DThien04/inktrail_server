...
const formatStory = (story) => ({
  ...
  moderation_reason: story.moderationReason ?? null,
  moderation_confidence: story.moderationConfidence ?? null,
  moderation_categories: story.moderationCategories ?? null,
...
const parseNonNegativeInt = (value, fieldName) => {
  ...
  if (!Number.isInteger(num) || num < 0) {
    throw new Error(`${fieldName} ph?i là s? nguyên không âm`);
  }
  return num;
};
...
const parseSearchLimit = (value) => {
  ...
  const num = Number(value);
  if (!Number.isInteger(num) || num <= 0) {
    throw new Error("limit ph?i là s? nguyên duong");
  }
...
const parseRatingScore = (value) => {
  ...
  if (!Number.isInteger(num)) {
    throw new Error("rating ph?i là s? nguyên");
  }
...
const parseRatingContent = (value) => {
  ...
  if (!normalized) {
    throw new Error("N?i dung dánh giá không du?c d? tr?ng");
  }
  if (normalized.length > 1000) {
    throw new Error("N?i dung dánh giá t?i da 1000 ký t?");
  }
  return normalized;
};
...
const buildSearchOrderBy = (sort) => {
  ...
  if (!SEARCH_SORTS.has(normalizedSort)) {
    throw new Error("sort không h?p l?");
  }
...
const parseGenreIdsInput = (genreIds) => {
  ...
  try {
    ...
    throw new Error("genre_ids ph?i là m?ng id h?p l?");
  }
};
...
const buildStoryGenreCreateData = async (genreIds) => {
  ...
  if (genres.length !== genreIds.length) {
    throw new Error("Có th? lo?i không t?n t?i ho?c dã b? ?n");
  }
...
const parseTagIdsInput = (tagIds) => {
  ...
  try {
    ...
    throw new Error("tag_ids ph?i là m?ng id h?p l?");
  }
};
  ...
  const normalizeNames = (values) => {
    ...
    for (const value of values) {
      ...
      if (normalizedValue.length > MAX_TAG_NAME_LENGTH) {
        throw new Error(`Tag t?i da ${MAX_TAG_NAME_LENGTH} ký t?`);
      }
...
const buildStoryTagCreateData = async (tagIds) => {
  ...
  if (tags.length !== tagIds.length) {
    throw new Error("Có tag không t?n t?i ho?c dã b? ?n");
  }
  ...
  if (tagNames?.length) {
    ...
    for (const item of normalizedNameEntries) {
      ...
      if (existing && !existing.isActive) {
        throw new Error(`Tag "${existing.name}" dang b? ?n, hãy dùng tag khác`);
      }
...
const ensureStoryOwnerOrAdmin = ({ story, requester }) => {
  ...
  if (!isOwner && !isAdmin) {
    throw new Error("B?n không có quy?n thao tác truy?n này");
  }
};
...
const createStory = async ({
  ...
  if (!normalizedTitle) throw new Error("Tiêu d? truy?n không du?c d? tr?ng");
  if (normalizedTitle.length > 200) throw new Error("Tiêu d? truy?n t?i da 200 ký t?");

  const normalizedDescription = normalizeText(description);
  if (normalizedDescription.length > 5000) {
    throw new Error("Mô t? truy?n t?i da 5000 ký t?");
  }

  const normalizedStatus = normalizeText(status) || "draft";
  if (!ALLOWED_STORY_STATUSES.has(normalizedStatus)) {
    throw new Error("Tr?ng thái truy?n không h?p l?");
  }
  ...
  for (const story of normalizedStories) {
    if (story.status === "published" && story.chapter_count === 0) {
      needsAttention.push({
        ...
        title: story.title,
        slug: story.slug,
        reason: "Truy?n dang phát hành nhung chua có chuong nào.",
      });
      continue;
    }
    if (story.status === "published" && story.read_count === 0) {
      needsAttention.push({
        ...
        title: story.title,
        slug: story.slug,
        reason: "Truy?n dã phát hành nhung chua có lu?t d?c.",
      });
      continue;
    }
    if (story.status === "published" && story.rating_count > 0 && story.rating < 3) {
      needsAttention.push({
        ...
        title: story.title,
        slug: story.slug,
        reason: "Ði?m dánh giá trung bình th?p hon 3.0.",
      });
    }
  }
...
const ensureStoryExists = async (storyId) => {
  ...
  if (!story) throw new Error("Không tìm th?y truy?n");
...
const validateCommentContent = (content) => {
  ...
  if (!normalizedContent) throw new Error("N?i dung bình lu?n không du?c d? tr?ng");
  if (normalizedContent.length > 2000) {
    throw new Error("N?i dung bình lu?n t?i da 2000 ký t?");
  }
...
const upsertStoryRating = async ({ storyId, requester, score, content }) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (existing.editCount >= 1) {
      throw new Error("B?n ch? có th? s?a dánh giá m?t l?n");
    }
  ...
  if (result.created && requester.id !== story.authorId) {
    await notificationService.createNotification({
      ...
      storyId: story.id,
      type: "system",
      title: `${getRequesterDisplayName(requester)} dã dánh giá truy?n ${story.title}`,
...
const getSimilarStories = async ({ storyId, requester, limit }) => {
  ...
  if (!fullBaseStory) throw new Error("Không tìm th?y truy?n");
  ...
  const coLikeRows = await prisma.chapterLike.findMany({
    where: {
      ...
      chapter: {
        ...
        story: { status: "published", isHidden: false },
      },
    },
    select: {
      userId: true,
...
const updateStory = async ({
  ...
  if (story.status === "published" && isEditingContent) {
    throw new Error("Truy?n dã xu?t b?n, hãy dua v? b?n nháp tru?c khi ch?nh s?a");
  }
  ...
  if (!Object.keys(data).length) {
    if (
      ...
      throw new Error("Không có d? li?u h?p l? d? c?p nh?t");
    }
  }
...
const deleteStory = async ({ storyId, requester }) => {
  ...
  return { message: "Xóa truy?n thành công" };
};
